...
export async function connect() {
	const runtime = JSON.parse(
		await readFile(resolve(root, ".personal-data/runtime.json"), "utf8"),
	);
	if (runtime.workspace !== root.replace(/\/$/, ""))
		throw new Error("Wrong workspace runtime");
	const targets = await fetch(
		`http://127.0.0.1:${runtime.debugPort}/json/list`,
	).then((r) => r.json());
	...
	if (!target)
		throw new Error("This workspace's desktop renderer is not ready");
      ...
      controls: [...document.querySelectorAll('button, a, input, select')]
        ...
        .map(el => ({tag:el.tagName,id:el.id,text:el.tagName==='INPUT'?'':el.textContent.trim().slice(0,100),
          label:el.getAttribute('aria-label'),placeholder:el.getAttribute('placeholder'),type:el.getAttribute('type')}))
			...
			result = await client.send("Input.insertText", { text: argument });
		else if (action === "evaluate") result = await client.evaluate(argument);
		else if (action === "screenshot")
			result = await client.screenshot(argument);
