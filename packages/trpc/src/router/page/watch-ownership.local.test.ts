...
import { closeDatabase, initializeDatabase } from "@superset/db/client";
import { and, eq, sql } from "drizzle-orm";

describe("page watch ownership against the local database", () => {
	let db: typeof import("@superset/db/client").db;
	...
	beforeAll(async () => {
		directory = await mkdtemp(join(tmpdir(), "superset-page-watch-"));
		process.env.SUPERESTSET_DATA_DIR = directory;
		process.env.SUPERESTSET_MIGRATIONS_DIR = resolve(
			import.meta.dir,
			"../../../../db/drizzle",
		);
		await initializeDatabase();
		({ db } = await import("@superset/db/client"));
		schema = await import("@superset/db/schema");
		watch = await import("./watch-ownership");
		const [org] = await db
			.insert(schema.organizations)
			.values({ name: "Page watch fixture", slug: "page-watch-fixture" })
			.returning({ id: schema.organizations.id });
		if (!org) throw Error("Organization fixture missing");
